 ...
 *
 * This object should be instantiated once per webpage on your project, at the
 * very beginning of the webpage, and then used for every subsequent document
 * query.
 */
public class Api {
	...
	public static class Error extends RuntimeException {
		...
		public enum Code {
			MALFORMED_URL, AUTHORIZATION_NEEDED, INVALID_TOKEN, TOO_MANY_REQUESTS, UNEXPECTED
		}
	...
	/**
	 * Entry point to get an {@link Api} object. Example:
	 * <code>API api = API.get("https://lesbonneschoses.prismic.io/api", null);</code>
	 *
	 * @param url
	 *            the endpoint of your prismic.io content repository, typically
	 *            https://yourrepoid.prismic.io/api
	...
	/**
	 * Entry point to get an {@link Api} object. Example:
	 * <code>API api = API.get("https://lesbonneschoses.prismic.io/api");</code>
	 *
	 * @param url
	 *            the endpoint of your prismic.io content repository, typically
	 *            https://yourrepoid.prismic.io/api
	...
	/**
	 * Constructor to build a proper {@link Api} object. This is not to build an
	 * {@link Api} object from an {@link Api} endpoint (the most usual case). In
	 * this case, you'll use the <code>API.get</code> method.
	 *
	 * @param apiData
	 *            the data retrieved from the API document, ready to be stored
	 *            in memory
	 * @param accessToken
	 *            Your Oauth access token if you wish to use one (to access
	 *            future content releases, for instance)
	...
	/**
	 * From a properly built {@link Api} object, returns the ref IDs (points in
	 * a prismic.io repository's timeline, whether in the past, in the present,
	 * or in the future) to which the passed credentials give access.
	 *
	...
	/**
	 * From a properly built {@link Api} object, returns the ref with the
	 * corresponding label
	 *
	 * @param label
	 *            the ref label as defined in the Writing Room
	 * @return the Ref, or null if not found
	 */
	public Ref getRef(String label) {
		for (Ref ref : apiData.getRefs()) {
			if (ref.getLabel().equals(label)) {
				return ref;
			}
		}
		return null;
	...
	/**
	 * From a properly built {@link Api} object, returns the map associating the
	 * bookmark names to their document IDs.
	 *
	 * Therefore, to get a bookmarked document's ID, it will look like this:
	 * <code>api.getBookmarks().get("home")</code>
	 *
	...
	/**
	 * From a properly built {@link Api} object, returns the map associating the
	 * type names to their readable text.
	 *
	 * Therefore, to get a type's readable text, it will look like this:
	 * <code>api.getTypes().get("blog")</code>
	 *
	...
	/**
	 * From a properly built {@link Api} object, returns the list of available
	 * tags.
	 *
	 * Therefore, to get all available tags, it will look like this:
	 * <code>api.getTags()</code>
	 *
	...
	/**
	 * From a properly built {@link Api} object, returns the Map of available
	 * form names.
	 *
	...
	/**
	 * From a properly built {@link Api} object, return a Form object that will
	 * allow to perform queries. Currently, all the forms offered by prismic.io
	 * are SearchForms, but this will change.
	 *
	 * To use it: <code>api.get("everything").query(.....)......</code>
	 *
	 * @param form
	 *            the name of the form to query on
	...
	/**
	 * From a properly built {@link Api} object, returns the ref ID (points in a
	 * prismic.io repository's timeline, whether in the past, in the present, or
	 * in the future) of the master ref (the one presently live).
	 *
	...
	/**
	 * Start a query defaulting on the master reference (you can still override
	 * it)
	 */
	 ...
	 * 
	 * @param token
	 *            as received from Prismic server to identify the content to
	 *            preview
	 * @param linkResolver
	 *            the link resolver to build URL for your site
	 * @param defaultUrl
	 *            the URL to default to return if the preview doesn't correspond
	 *            to a document (usually the home page of your site)
	 * @return the URL you should redirect the user to preview the requested
	 *         change
	 */
	...
	public static class ApiData {
		...
		public ApiData(List<Ref> refs, Map<String, String> bookmarks, Map<String, String> types, List<String> tags, Map<String, Form> forms,
				Experiments experiments, String oauthInitiateEndpoint, String oauthTokenEndpoint) {
