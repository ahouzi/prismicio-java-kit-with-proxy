 ...
 * {@link Form.SearchForm}. If your need is simply to query content, you
 * shouldn't need to look into this class, and use the methods in
 * {@link Form.SearchForm} instead.
 */
public class Form {
	...
	/**
	 * The object you will use to perform queries. At the moment, only queries
	 * of the type "SearchForm" exist in prismic.io's APIs. There is one named
	 * "everything", that allow to query through the while repository, and there
	 * is also one per collection created by prismic.io administrators in the
	 * writing-room.
	 *
	 * From an {@link Api} object, you get a SearchForm form like this:
	 * <code>api.getForm("everything");</code>
	 *
	 * Then, from a SearchForm form, you query like this:
	 * <code>search.query("[[:d = at(document.type, "Product")]]").ref(ref).submit();</code>
	 */
	public static class SearchForm {
		...
		/**
		 * Allows to set one of the form's fields, such as "q" for the query
		 * field, or the "ordering" field, or the "pageSize" field. The field
		 * must exist in the RESTful description that is in the /api document.
		 * To be on the safe side, you should use the specialized methods, and
		 * use <code>searchForm.orderings(o)</code> rather than
		 * <code>searchForm.set("orderings", o)</code> if they exist.
		 *
		 * @param field
		 *            the name of the field to set
		 * @param value
		 *            the value with which to set it
		 ...
		 *
		 * This is mandatory to submit a query; if you call
		 * <code>api.getForm("everything").submit();</code>, the kit will
		 * complain!
		 *
		 * Please do not forge the ref dynamically in this call, like this:
		 * <code>ref(api.getMaster())</code>. Prefer to set a ref variable once
		 * for your whole webpage, and use that variable in this method:
		 * <code>ref(ref)</code>. That way, you can change this variable's
		 * assignment once, and trivially set your whole webpage into the future
		 * or the past.
		 *
		 * @param ref
		 *            the ref object representing the ref on which you wish to
		 *            query
		 * @return the current form, in order to chain those calls
		 */
		 ...
		 *
		 * Please do not forge the ref dynamically in this call, like this:
		 * <code>ref(api.getMaster().getRef())</code>. Prefer to set a ref
		 * variable once for your whole webpage, and use that variable in this
		 * method: <code>ref(ref)</code>. That way, you can change this
		 * variable's assignment once, and trivially set your whole webpage into
		 * the future or the past.
		 *
		 * @param ref
		 *            the ID of the ref on which you wish to query
		 * @return the current form, in order to chain those calls
		 */
		 ...
		 *
		 * The default value is 20; a call with a different page size will look
		 * like:
		 * <code>api.getForm("everything").pageSize("15").ref(ref).submit();</code>
		 * .
		 *
		 * @param pageSize
		 *            the size of the pagination you wish
		 * @return the current form, in order to chain those calls
		 */
		 ...
		 * (do remember that the default size of a page is 20, you can change it
		 * with <code>pageSize</code>)
		 *
		 * @param page
		 *            the page number
		 * @return the current form, in order to chain those calls
		 ...
		 *
		 * The default value is 1; a call for a different page will look like:
		 * <code>api.getForm("everything").page(2).ref(ref).submit();</code> (do
		 * remember that the default size of a page is 20, you can change it
		 * with <code>pageSize</code>)
		 *
		 * @param page
		 *            the page number
		 ...
		 * Read prismic.io's API documentation to learn more about how to write
		 * orderings.
		 *
		 * @param orderings
		 *            one or more ordering, as the name of a field optionally
		 *            followed by "desc"
		 * @return the current form, in order to chain those calls
		 */
		 ...
		 * Look up prismic.io's documentation online to discover the possible
		 * query predicates.
		 *
		 * Beware: a query is a list of predicates, therefore, it always starts
		 * with "[[" and ends with "]]".
		 *
		 * @param q
		 *            the query to pass
		...
		/**
		 * Allows to set the query field of the current form, using Predicate
		 * objects. Example:
		 * <code>search.query(Predicates.at("document.type", "Product"));</code>
		 * See io.prismic.Predicates for more helper methods.
		 *
		 * @param predicates
		 *            any number of predicate, is more than one is provided
		 *            documents that satisfy all predicates will be returned
		 *            ("AND" query)
		 * @return the current form, in order to chain those calls
		 */
		 ...
		 *
		 * Please make sure you're set a ref on this SearchForm form before
		 * querying, or the kit will complain!
		 *
