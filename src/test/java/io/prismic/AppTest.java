...
public class AppTest {
	...
	public void apiIsInitialized() {
		Assert.assertTrue("Api object is null.", micro_api != null);
		Assert.assertEquals("Api object does not return the right master ref.", micro_api.getMaster().getRef(), "WH8MzyoAAGoSGJwT");
	}

	/**
	 * Make sure a call to a private repository without a token returns the
	 * expected error
	 */
	...
	public void apiQueryWorks() {
		Assert.assertEquals("SearchForm query does not return the right amount of documents.", 6, micro_api.getForm("doc").ref(micro_api.getMaster())
				.submit().getResults().size());
	}

	@Test
	public void proxySupportWorks() throws UnknownHostException {
	...
	/**
	 * Tests whether complex blog posts serialize well into HTML. This allows to
	 * test many fragment types in one shot.
	 */
	// @Test
	...
	// }, new HtmlSerializer() {
	// @Override
	// public String serialize(Fragment.StructuredText.Element element, String
	// content) {
	// if (element instanceof Fragment.StructuredText.Block.Image) {
	// Fragment.StructuredText.Block.Image image =
	// (Fragment.StructuredText.Block.Image)element;
	...
	public void groupFragments() {
		Document docchapter = micro_api.query("[[:d = at(document.type, \"docchapter\")]]").set("orderings", "[my.docchapter.priority]").submit()
				.getResults().get(0);
		...
		Assert.assertEquals("HTML serialization of docchapter \"Using meta-micro\"", docchapter_expected, docchapter_retrieved);

		Fragment.Group docchapterGroup = docchapter.getGroup("docchapter.docs");
		Assert.assertTrue("Group finds the proper amount of elements", docchapterGroup.getDocs().size() == 2);

		Assert.assertEquals("Properly browsing the group until inside a subfragment", ((Fragment.DocumentLink) docchapterGroup.getDocs().get(0)
				.getLink("linktodoc")).getId(), "UrDofwEAALAdpbNH");
	}

	// @Test
	// public void preformattedSerialization() {
	// Document installingMetaMicro =
	// micro_api.query("[[:d = at(document.id, \"UrDejAEAAFwMyrW9\")]]")
	// .submit().getResults().get(0);
	// String retrieved =
	// installingMetaMicro.getStructuredText("doc.content").asHtml(new
	// SimpleLinkResolver() {
	// public String resolve(Fragment.DocumentLink link) {
	// return "/"+link.getId()+"/"+link.getSlug();
	// }
	...
	public void linkedDocuments() {
		...
		Assert.assertEquals("Have one linked document", 1, documents.get(0).getLinkedDocuments().size());
	}

	@Test
	public void pagination() {
		Assert.assertEquals("Page number is right if page 1 requested", 1, micro_api.query().submit().getPage());
		Assert.assertEquals("Results per page is right if page 1 requested", 20, micro_api.query().submit().getResultsPerPage());
		Assert.assertEquals("Total results size is right if page 1 requested", 24, micro_api.getForm("everything").ref(micro_api.getMaster())
				.submit().getTotalResultsSize());
		Assert.assertEquals("Total pages is right if page 1 requested", 2, micro_api.getForm("everything").ref(micro_api.getMaster()).submit()
				.getTotalPages());
		Assert.assertNull("Previous page is right if page 1 requested", micro_api.getForm("everything").ref(micro_api.getMaster()).submit()
				.getPrevPage());
		Assert.assertNull("Next page is right if page 2 requested", micro_api.getForm("everything").set("page", 2).ref(micro_api.getMaster())
				.submit().getNextPage());
	}
	...
	// List<Document> documents = lbc_api.query(Predicates.at("document.id",
	// "UlfoxUnM0wkXYXbt"))
	// .fetchLinks("blog-post.author").submit().getResults();
	// Fragment.DocumentLink link =
	// (Fragment.DocumentLink)documents.get(0).getLink("blog-post.relatedpost[0]");
	// Assert.assertEquals(
	...
	// Api api = Api.get("https://lesbonneschoses.cdn.prismic.io/api", null, new
	// Cache.NoCache(), null);
	// Assert.assertEquals(
